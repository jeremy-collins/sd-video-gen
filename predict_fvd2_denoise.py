        ...
        for (ind, (index_list, batch)) in enumerate(test_loader):
            ...
            for iteration in range(args.pred_frames):
                ...
                if args.denoise:
                    ...
                    img_to_save = np.array(noisy_img[0])
                    pr_val = cv2.imwrite(os.path.join('outputs_pred', str(args.config) + '_' + str(args.index)+ '_' + str(args.mode) + '_25Nov4', str(ind) + '_' + str(i)+ '.png'), img)
