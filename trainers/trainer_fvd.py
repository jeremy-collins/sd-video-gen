...
from models.contrastive_loss import BiPatchNCE
import fvd
import traceback
import sd_utils
...
class Trainer():
    ...
    def validation_loop(self, model, loss_fn, dataloader, frames_to_predict, compute_fvd, params_for_fvd):  
        model.eval()
        total_loss = 0
        mse_loss = 0
        gdl_loss = 0
        ...
        with torch.no_grad():
            for j, (index_list, batch) in enumerate(tqdm(dataloader)):
                ...
                #print('y_expected[-frames_to_predict:]', y_expected[-frames_to_predict:].shape)
                #print('batch', batch.shape)
                #print('pred', pred.shape)
                #grnd_truth = y_expected[-frames_to_predict:].reshape((y_expected[-frames_to_predict:].shape[0], y_expected[-frames_to_predict:].shape[1], 4, config.FRAME_SIZE // 8, config.FRAME_SIZE // 8))
                #grnd_truth = grnd_truth.permute(1, 0, 2, 3, 4)
                #print('grnd_truth', grnd_truth.shape)

                if compute_fvd:
                    groundtruth_frames = params_for_fvd['transform_to_224'](batch)
                    #print('groundtruth_frames', groundtruth_frames.shape)
                    fvd.update_stats_for_sequence(params_for_fvd['dectector_for_fvd'], params_for_fvd['stats_groundtruth'], groundtruth_frames)

                    pred_frames = []
                    pred = pred.permute(1, 0, 2)
                    #print('pred ', pred.shape)
                    for i, latent in enumerate(pred):
                        # latent = latent.reshape((1, 4, 8, 8))
                        latent = latent.reshape((pred.shape[1], 4, config.FRAME_SIZE // 8, config.FRAME_SIZE // 8))
                        img = sd_utils.decode_img_latents(latent)
                        #img = np.array(img[0])
                        pred_frames.append(img)

                    predicted_frames = torch.from_numpy(np.array(pred_frames))
                    predicted_frames = predicted_frames.reshape((predicted_frames.shape[0], -1, config.FRAME_SIZE, config.FRAME_SIZE, 3))
                    #print('predicted_frames ', predicted_frames.shape)
                    predicted_frames = params_for_fvd['transform_to_224'](predicted_frames)
                    #print('predicted_frames', predicted_frames.shape)
                    fvd.update_stats_for_sequence(params_for_fvd['dectector_for_fvd'], params_for_fvd['stats_predicted'], predicted_frames)
                    #print('predicted_frames', predicted_frames.shape)

        if compute_fvd:
            fvd_score = fvd.compute_fvd(params_for_fvd['stats_predicted'], params_for_fvd['stats_groundtruth'])
            print('FVD : ', fvd_score)
        ...
        if compute_fvd:
            wandb.log({'FVD score': fvd_score})
            
        return val_loss

    def fit(self, model, opt, scheduler, loss_fn, train_dataloader, val_dataloader, frames_to_predict, compute_fvd=False, params_for_fvd=None): 
        print("Training and validating model")
            
        train_loss = self.train_loop(model, opt, scheduler, loss_fn, train_dataloader, frames_to_predict)
        validation_loss = self.validation_loop(model, loss_fn, val_dataloader, frames_to_predict, compute_fvd, params_for_fvd)
...
def main():
    try:
        ...
        trans224 = transforms.Compose([transforms.Lambda(lambda x: x.permute(0, 1, 4, 2, 3)),
                                                transforms.Lambda(lambda x: x.reshape(x.shape[0] * x.shape[1], x.shape[2], x.shape[3], x.shape[4])), 
                                                transforms.Resize(224), 
                                                transforms.Lambda(lambda x: x.reshape(-1, frames_to_predict, x.shape[1], x.shape[2], x.shape[3])),
                                                transforms.Lambda(lambda x: x.permute(0, 1, 3, 4, 2)),
                                                ])
        params_for_fvd = {'dectector_for_fvd': fvd.load_detector(),
            'stats_groundtruth': fvd.get_FeatureStats(),
            'stats_predicted': fvd.get_FeatureStats(),
            'transform_to_224': trans224}
...
# 10 
        ...
        for epoch in range(1, epochs+1):
            ...
            train_loss, validation_loss = trainer.fit(model=model, opt=opt,  scheduler=scheduler, loss_fn=loss_fn, train_dataloader=train_loader, val_dataloader=test_loader, frames_to_predict=frames_to_predict, compute_fvd=(epoch%5==1), params_for_fvd=params_for_fvd)
