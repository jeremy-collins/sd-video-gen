...
import math
import numpy as np
from models.transformer_text import Transformer
...
class Trainer():
    ...
    def train_loop(self, model, opt, scheduler, loss_fn, dataloader, frames_to_predict, idx_to_class):
        ...
        for i, (index_list, batch) in enumerate(tqdm(dataloader)):
            ...
            pred = model(new_batch, cls_list, y_input, tgt_mask)
    ...
    def validation_loop(self, model, loss_fn, dataloader, frames_to_predict, idx_to_class):  
        model.eval()
        total_loss = 0
        mse_loss = 0
        gdl_loss = 0
    ...
    def fit(self, model, opt, scheduler, loss_fn, train_dataloader, val_dataloader, frames_to_predict, idx_to_class): 
        print("Training and validating model")
            
        train_loss = self.train_loop(model, opt, scheduler, loss_fn, train_dataloader, frames_to_predict, idx_to_class)
        validation_loss = self.validation_loop(model, loss_fn, val_dataloader, frames_to_predict, idx_to_class)
...
def main():
    try:
        ...
        for epoch in range(1, epochs+1):
            ...
            train_loss, validation_loss = trainer.fit(model=model, opt=opt,  scheduler=scheduler, loss_fn=loss_fn, train_dataloader=train_loader, val_dataloader=test_loader, frames_to_predict=frames_to_predict, idx_to_class=idx_to_class)
